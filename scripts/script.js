...
let postsArray = [
  ["https://solscan.io/account/AxiomRXZAq1Jgjj9pHmNqVP7Lhu67wLXZJZbaK87TTSk", encodeURI("Based on Error and proofs")],
];
...
let footerHTML = "<p><a href='https://jwhighwind.xyz/secret/sphear.html'>たくさんの冒険をさせてもらったこどもの頃 <br> いま、大人になった僕らが作られる新しい物語はなんだろう？</a></p>";
...
function formatPostLink(i) {
  ...
  const href = postsArray[i][0];
  const isExternal = href.startsWith('http');
  
  // For external links, use the URL directly without prepending relativePath
  if (isExternal) {
    return '<li><a href="' + href + '" target="_blank" rel="noopener noreferrer">' + postTitle_i + '</a></li>';
  } else {
    // For internal links, prepend the relativePath
    if (postDateFormat.test(postsArray[i][0].slice(6, 17))) {
...
let recentPostListHTML = "<h3>Latest tx:</h3><ul>";
